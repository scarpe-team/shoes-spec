...
BATCH_FILE = File.join(__dir__, "batches/batch_13.txt")
...
def run_test(sspec_path)
  ...
  with_tempfiles([
    ...
    sspec_file = File.join(__dir__, "implementations/niente/sspec_batch13.json")
    
    ENV["SHOES_SPEC_TEST"] = test_file
    ENV["SCARPE_DISPLAY_SERVICE"] = "niente"
    ENV["SHOES_MINITEST_EXPORT_FILE"] = sspec_file
    ENV["SHOES_MINITEST_CLASS_NAME"] = "Batch13"
    ...
    if result && File.exist?(sspec_file)
      ...
      {name: test_name, status: status, assertions: json[0]["assertions"], failures: json[0]["failures"].map { |f| f[0] }, raw_failures: json[0]["failures"]}
    else
...
if fail_count > 0
  ...
  RESULTS.select { |r| r[:status] == :fail }.each do |r|
    puts "  #{r[:name]}:"
    r[:raw_failures]&.each do |f|
      if f[1]
        begin
          parsed = JSON.parse(f[1])
          puts "    #{parsed['m']&.split("\n")&.first}"
        rescue
          puts "    #{f[0]}"
        end
