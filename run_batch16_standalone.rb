#!/usr/bin/env ruby
# Run batch 16 tests - Standalone version
...
BATCH_FILE = File.join(SHOES_SPEC_DIR, "batches/batch_16.txt")
...
def run_test(sspec_path)
  ...
  begin
    ...
    sspec_file = File.join(SHOES_SPEC_DIR, "implementations/niente/sspec_batch16.json")
    ...
    env = {
      ...
      "SHOES_MINITEST_CLASS_NAME" => "Batch16",
...
# Write JSON results
results_json = File.join(SHOES_SPEC_DIR, "implementations/niente/batch_16_results.json")
File.write(results_json, JSON.pretty_generate(RESULTS))
puts "\nResults written to: #{results_json}"

# Generate markdown table
puts "\n" + "=" * 60
puts "MARKDOWN TABLE FOR SESSION LOG"
puts "=" * 60
puts "| Test | Status |"
puts "|------|--------|"
RESULTS.each do |r|
